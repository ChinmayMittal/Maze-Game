...
Dot :: Dot( LTexture &myTexture , LWindow &window , int playerHeight , int playerWidth  , int right , int left , int top , int bottom , int setVelocity   )  : mTexture(myTexture), mWindow(window)
{
...
void Dot::handleEvent(SDL_Event &e)
{
    ...
    else if (e.type == SDL_KEYUP && e.key.repeat == 0)
    {
        ...
        {
        case SDLK_UP:
            if(mVelY != 0 ) mVelY += DOT_VEL;
            break;
        case SDLK_DOWN:
            if(mVelY != 0 ) mVelY -= DOT_VEL;
            break;
        case SDLK_LEFT:
            if(mVelX != 0 ) mVelX += DOT_VEL;
            break;
        case SDLK_RIGHT:
            if(mVelX != 0 ) mVelX -= DOT_VEL;
            break;
...
void Dot::move()
{
    ...
    if ((mBox.x < 0) || (mBox.x + playerWidth > mWindow.getLevelWidth()) /* || touchesWall(mBox, tiles) */)
    {
        // move back
        mBox.x -= mVelX;
    }
    ...
    if ((mBox.y < 0) || (mBox.y + playerHeight > mWindow.getLevelHeight()) /* || touchesWall(mBox, tiles) */)
    {
        // move back
        mBox.y -= mVelY;
    }
...
void Dot::setCamera(SDL_Rect &camera)
{
    ...
    camera.x = (mBox.x + playerWidth / 2) - mWindow.getWidth() / 2;
    camera.y = (mBox.y + playerHeight / 2) - mWindow.getHeight() / 2;
